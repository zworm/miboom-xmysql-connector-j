...
public class Buffer implements PacketPayload {
	...
	public String toString() {
		...
		this.position = 0;
		String dumped = StringUtils.dumpAsHex(readBytes(StringLengthDataType.STRING_FIXED, numBytesToDump),
				numBytesToDump);
	...
	/**
	 * To be called after write operations to ensure that payloadLength contains the
	 * real size of written data.
	 */
	...
	public final long readInteger(IntegerDataType type) {
		...
		// zzf: avoid ArrayIndexOutOfBoundsException cause
		// ResultSetImpl#getString/getTimestamp... no judgment if
		// null：thisRow.getNull(columnIndex)
	...
	public byte[] readBytes(StringSelfDataType type) {
		...
		case STRING_LENENC:
			...
			return l == NULL_LENGTH ? null
					: (l == 0 ? Constants.EMPTY_BYTE_ARRAY : readBytes(StringLengthDataType.STRING_FIXED, (int) l));
