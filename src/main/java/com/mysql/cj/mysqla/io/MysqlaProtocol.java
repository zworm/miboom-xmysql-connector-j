...
//        return serverCapabilities;
    	...
    	// zzf 6.06-->8.11: Fix ArrayIndexOutOfBoundsException @ MysqlaCapabilities.setInitialHandshakePacket-->setThreadId
      // Read the first packet
    	PacketPayload buf = readPacket(null);

      // Server Greeting Error packet instead of Server Greeting
      if((buf.getByteBuffer()[0]& 0xff) == PacketPayload.TYPE_ID_ERROR) {
          rejectProtocol(buf);
      }
